document.addEventListener('DOMContentLoaded', () => {
  ...
  let currentPlayer = 1;
   ...
   for (var i = 0, len = squeres.length; i < len; i++){
    (function(index){
      squeres[i].onclick = function(){
         if(squeres[index + 7].classList.contains('taken')){
          if(currentPlayer === 1){
            squeres[index].classList.add('taken');
            squeres[index].classList.add('player-one');
            currentPlayer = 2;
            displayCurrentPlayer.innerHTML = currentPlayer;
          } else if(currentPlayer === 2){
            squeres[index].classList.add('taken');
            squeres[index].classList.add('player-two');
            currentPlayer = 1;
            displayCurrentPlayer.innerHTML = currentPlayer;
          }
        } else {
  ...
  function checkBoard(){
    const winningArrays = [
     ...
     for(let y = 0; y < winningArrays.length; y++) {
        const squere1 = squeres[winningArrays[y][0]];
        const squere2 = squeres[winningArrays[y][1]];
        const squere3 = squeres[winningArrays[y][2]];
        const squere4 = squeres[winningArrays[y][3]];
        console.log(squeres[winningArrays[y][0]])
      if(squere1.classList.contains('player-one') &&
         squere2.classList.contains('player-one') &&
         squere3.classList.contains('player-one') &&
         squere4.classList.contains('player-one')) {
         result.innerHTML = 'Player one wins!'
           }
       else if (squere1.classList.contains('player-two') &&
           squere2.classList.contains('player-two') &&
           squere3.classList.contains('player-two') &&
           squere4.classList.contains('player-two')) {
           result.innerHTML = 'Player two wins!'
       }
     }
  }
  squeres.forEach(squere => squere.addEventListener('click', checkBoard))
})
